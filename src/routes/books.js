...
router.post('/import', async (req, res) => {
  try {
    ...
    for (const b of books) {
      ...
      // derive owned from old status field
      const oldStatus = b.status || '';
      const owned = b.owned || oldStatus === 'owned-unread' || oldStatus === 'read' || oldStatus === 'reading';
      const status = oldStatus === 'owned-unread' ? 'unread' :
                     oldStatus === 'want-to-buy' ? 'unread' :
                     oldStatus === 'want-to-read' ? 'unread' :
                     oldStatus || 'unread';
